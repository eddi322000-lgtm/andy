...
namespace console {
    ...
    void status_bar(std::function<void(std::string &)> update) {
        std::lock_guard<std::mutex> lock(g_console_mutex);
        ...
        // Move cursor to bottom line, column 0
        ...
        // Write status + clear rest of line
        fprintf(out, "%s", status.c_str());
        fprintf(out, "\033[K"); // Clear to end of line
        fflush(out);
    }
    ...
    void clear_status_bar() {
        ...
        fprintf(out, "\033[%d;1H\033[K", rows);
        fflush(out);
    }
}
