...
// ---------------------------------------------------------------------------
static bool is_path_in_dir(const std::string & search_path, const std::string & working_dir) {
    try {
        fs::path abs_workdir = fs::absolute(fs::canonical(working_dir));
        fs::path abs_search = fs::absolute(search_path);

        // Resolve ALL symlinks to prevent symlink-based path traversal
        if (fs::exists(abs_search)) {
            abs_search = fs::canonical(abs_search);
        } else if (abs_search.has_parent_path() && fs::exists(abs_search.parent_path())) {
            abs_search = fs::canonical(abs_search.parent_path()) / abs_search.filename();
        }

        std::string search_str = abs_search.string();
        std::string workdir_str = abs_workdir.string();

        if (search_str == workdir_str) {
            return true;
        }
        if (search_str.size() > workdir_str.size() &&
            search_str.substr(0, workdir_str.size() + 1) == workdir_str + "/") {
            return true;
        }
    } catch (...) {
        return false;
...
// ---------------------------------------------------------------------------
// Security: ReDoS protection — limit regex compilation and execution time
// ---------------------------------------------------------------------------
static constexpr int32_t k_regex_timeout_ms = 500;

static bool regex_search_with_timeout(const std::string & text, const std::regex & re) {
    // Use a timed approach: regex_search can be very slow on pathological inputs
    // We rely on std::regex's internal timeout (if available) or limit input size
    if (text.size() > 10000) {
        // Truncate very long inputs for regex matching to prevent ReDoS
        return std::regex_search(text.substr(0, 10000), re);
    }
...
static tool_result glob_execute(const json & args, const tool_context & ctx) {
    ...
    // Security: Prevent path traversal — search must be inside working_dir
    if (!is_path_in_dir(base_path.string(), ctx.working_dir)) {
        return {false, "", "Path traversal detected: search path must be within the working directory"};
    }
