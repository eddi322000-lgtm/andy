...
enum class permission_type {
    ...
    MCP_TOOL,      // External MCP server tool (unknown capabilities — always prompt)
};
...
private:
    ...
    // Normalize a shell command to defeat trivial obfuscation (backslash escapes, ${IFS}, whitespace runs)
    static std::string normalize_shell_command(const std::string & cmd);
    // Check if a shell command references sensitive system paths (/etc, ~/.ssh, etc.)
    static bool contains_sensitive_path_reference(const std::string & cmd);
