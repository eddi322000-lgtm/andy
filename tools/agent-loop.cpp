...
{
    // Capture model info for status bar
    const auto & meta = backend_.meta();
    model_name_ = meta.model_name;
    n_ctx_ = meta.n_ctx;
    ...
    std::string system_prompt = R"(You are andy-agent, a powerful local AI coding assistant running on llama.cpp.
...
agent_loop_result agent_loop::run_streaming(
    ...
    while (config_.max_iterations <= 0 || result.iterations < config_.max_iterations) {
        ...
        // Increment compaction cooldown counter each iteration
        compaction_iterations_++;
