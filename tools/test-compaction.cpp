...
static void test_estimate_tokens() {
    ...
    assert(tokens == 4); // 11 chars / 3 = 3.67, ceil = 4
    ...
    // Empty message: minimum 1 token (message overhead always exists)
    json empty = {{"role", "user"}, {"content", ""}};
    assert(compaction_estimate_tokens(empty) == 1);
...
static void test_estimate_total_tokens() {
    ...
    assert(total == 235); // 134 + 67 + 34 with chars/3
