...
std::optional<skill_metadata> skills_manager::parse_skill_single(const std::string & skill_file_path) {
    if (!fs::exists(skill_file_path) || !fs::is_regular_file(skill_file_path)) {
        return std::nullopt;
    }
    ...
    auto skill = parse_frontmatter(content, skill_file_path);
    if (!skill) {
        return std::nullopt;
    }

    // For single files, use the filename stem as skill_dir (no name validation)
    skill->skill_dir = fs::absolute(fs::path(skill_file_path).parent_path()).string();

    // No scripts support for single SKILL.md files
    return skill;
}
    ...
    for (const auto & search_path : search_paths) {
        ...
        try {
            for (const auto & entry : fs::directory_iterator(search_path)) {
                std::optional<skill_metadata> skill;
                ...
                if (entry.is_directory()) {
                    ...
                    skill = parse_skill(entry.path().string());
                } else if (entry.path().filename() == "SKILL.md") {
                    // Support single SKILL.md file (e.g., skills/tmp/SKILL.md)
                    skill = parse_skill_single(entry.path().string());
                }
