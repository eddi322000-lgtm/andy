    ...
    while (std::getline(file, line)) {
        ...
        if (type == "message") {
            if (entry.contains("message")) {
                const json & msg = entry["message"];
                // SECURITY: A tampered session file could inject a malicious
                // "system" role message (prompt injection). The new session
                // already sets its own system prompt, so drop any "system"
                // messages from the loaded session file.
                std::string role = msg.value("role", "");
                if (role == "system") {
                    LOG_WRN("Session file: dropped injected system message (prompt injection protection)\n");
                    continue;
                }
