    ...
    if (content.is_array()) {
        ...
        for (const auto & block : content) {
            if (block.contains("type") && block["type"] == "image_url") {
                // IMAGE SIZE ESTIMATION FIX: A base64 image can be enormous.
                // If we have the actual data_uri length, use it; otherwise
                // estimate conservatively to avoid OOM from under-estimation.
                if (block.contains("image_url") && block["image_url"].is_object() &&
                    block["image_url"].contains("url") && block["image_url"]["url"].is_string()) {
                    const std::string & url = block["image_url"]["url"].get<std::string>();
                    // data:image/...;base64,... — count actual bytes
                    chars += (int32_t) url.size();
                } else {
                    // Conservative fallback: 10MB base64 image
                    chars += 14000000; // ~13.3M chars for 10MB base64
                }
...
int32_t compaction_estimate_tokens(const json & msg) {
    ...
    // TOKEN ESTIMATION FIX: Code tokens average ~2.5 chars/token, not 4.
    // Using chars/3 as a middle ground gives better accuracy for mixed
    // code/text conversations without over-estimating for pure text.
    return std::max(1, (chars + 2) / 3);
}
