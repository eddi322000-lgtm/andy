...
std::string find_mcp_config(const std::string & working_dir) {
    // SECURITY: Do NOT auto-load mcp.json from the working directory.
    // A malicious repository could ship an mcp.json that executes arbitrary
    // commands (supply-chain attack). Only load the user's explicit config:
    //   ~/.andy-agent/mcp.json
    //
    // To opt in to a project-local config, the user can pass it explicitly
    // via --mcp-config <path> or copy it to ~/.andy-agent/mcp.json.
    (void)working_dir;

    // Check user config directory (explicitly user-created, trusted)
    const char * home = std::getenv("HOME");
    if (home) {
        fs::path user_config = fs::path(home) / ".andy-agent" / "mcp.json";
...
// ========================================================================
// Export Mode Implementation
// ========================================================================

void mcp_server_manager::start_export_mode(const std::string & server_name,
                                           const std::string & server_version) {
    if (export_mode_active_) {
        last_error_ = "Export mode already active";
        return;
    }
    ...
    tool_handler_t handler = [this](const std::string & tool_name, const json & arguments) -> json {
    ...
    export_server_ = std::make_unique<mcp_server>();
    export_server_->start(server_name, server_version, mcp_tools, std::move(handler));
    export_mode_active_ = true;
}

void mcp_server_manager::shutdown_export_mode() {
    if (!export_mode_active_) {
        return;
    }

    if (export_server_) {
        export_server_->shutdown();
    }
    export_server_.reset();
    export_mode_active_ = false;
}
