...
json mcp_client::read_message(int timeout_ms) {
    ...
    while (true) {
        ...
        // SECURITY / OOM FIX: Cap the read buffer at 1MB to prevent
        // unbounded memory growth from malformed or malicious MCP servers.
        if (read_buffer_.size() > 1024 * 1024) {
            last_error_ = "MCP server response too large (>1MB buffer)";
            return json();
        }
    }
}
