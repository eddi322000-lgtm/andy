...
static void test_skill_search_path_order() {
    ...
    assert(paths.size() >= 5);
    assert(paths[0] == "/work/.andy-agent/skills");
...
static void test_empty_working_dir_defaults_to_current_dir() {
    ...
    assert(paths.size() >= 3);
    assert(paths[0] == "./.andy-agent/skills");
...
static void test_discover_resources_respects_enable_flags() {
    const fs::path root = fs::temp_directory_path() / "andy-agent-resources-test";
    fs::remove_all(root);

    write_file(root / "AGENTS.md", "project instructions\n");
    write_file(root / ".andy-agent" / "skills" / "test-skill" / "SKILL.md",
               "---\n"
               "name: test-skill\n"
